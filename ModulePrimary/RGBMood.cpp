...
RGBMood::RGBMood(bool common_cathode, uint8_t rp, uint8_t gp, uint8_t bp){
  common_cathode_ = common_cathode;
  RGBMood(rp, gp, bp);
}
...
void RGBMood::tick() {
  ...
  if (pins_[0] > 0) {
    if(common_cathode_){
      ...
      analogWrite(pins_[0], 255-current_RGB_color_[0]);
      analogWrite(pins_[1], 255-current_RGB_color_[1]);
      analogWrite(pins_[2], 255-current_RGB_color_[2]);
    }
  }
}
