...
bool MAX5395::setWiperPosition(byte pos, bool confirm = false){
	Wire.beginTransmission(addr);
	Wire.write(WIPER);
	Wire.write(pos);
	Wire.endTransmission();
...
bool MAX5395::setChargePumpStatus(bool enabled, bool confirm = false){
...
void MAX5395::shutdown(Switch switchToOpen, WiperSetting setting){
	// Determine the command to send based on inputs
	byte cmd;

	if(switchToOpen == wiper)
		cmd = 0x84;
	else if(switchToOpen == low)
...
WiperSetting MAX5395::getWiperSetting(){
	byte c = getConfig();
